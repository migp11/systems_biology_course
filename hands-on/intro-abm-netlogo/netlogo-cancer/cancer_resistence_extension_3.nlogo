...
to load-expression-pheno-dict
  ...
  let csv-file-name "data/EV_profile_str_fitness_map.csv"
  if not file-exists? csv-file-name [
    user-message "No file csv-file-name exists!"
    stop
  ]
...
to seed-cells
  ...
  ask cells [
    ...
    [
       set expression-profile "11111111111111"
...
to setup
  ...
  set Monod_Ks 0.1
  set maintenance-requirement ( max-growth-rate * 0.5 )
...
to go
  ...
  ask cells [
    ...
    if fitness <= 2 [ set quiescent? true  ]
    ...
    [
      ...
      ifelse any? empty-patches and fitness = 3
      [ ;; There is space te growth
        set quiescent? false
        ;; Does the cell reach a critical volume to divide?
        let cell-size  get-cell-size size
        ifelse ( cell-size > random-normal critical-cell-size 1 )
        [ mitosis ]
        [
