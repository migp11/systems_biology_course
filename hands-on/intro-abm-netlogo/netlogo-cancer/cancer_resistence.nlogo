...
globals [
  ...
  ;; Cell
  cell-size ;; Initial cell volume
  contact-distance ;; Minimum distance at which growth can be inhibited
  move-step-size ;; size of the step in which cells move
  num-neighbor-inhiviting-growth ;; number of neighbor in the Minium radio that inhibit growth
  starvation-threshols ;; threshold below which cells die
  growth-nutrient-requirement ;; amount of nutrient required to growth
  maintenance-nutrient-requirement ;; amount of nutrient required to survive without growth
]
...
to setup
  ...
  set mutation-rate 0.001
  set cell-size 1
  set contact-distance 1
  set num-neighbor-inhiviting-growth 3
  set move-step-size cell-size * 0.05
  set starvation-threshols 0.0

  set growth-nutrient-requirement 0.03
  set maintenance-nutrient-requirement 0.01
...
to go
  ...
  ask cells [
    ...
    if ( nutrients < starvation-threshols or fitness <= 0)
      [ die ]
    ...
    ifelse (count other cells in-radius contact-distance) < num-neighbor-inhiviting-growth and fitness > 1
    [  set nutrients (nutrients - growth-nutrient-requirement) ;; nutrient consumption is higher when growing
       ifelse ( random-float 1 > (( cell-size * 2 ) - size) )
          [ mitosis ]
          [ set size size + ( growth-rate * fitness * 0.1) ] ]
    [  set nutrients (nutrients - maintenance-nutrient-requirement) ] ;; lower nutrient consumption on non-gorwing cells


    ;; Chek if there are neighbor cells too close, and move the opposite direction.
    if (any? other cells in-radius contact-distance)
      [ set heading towards one-of other cells in-radius contact-distance rt 180 fd move-step-size ]

    ;; Otherwise move randomly
    ; [ set heading random 360 fd move-step-size ]
...
to-report get-growth-rate [ umax ]
...
"" ""
PENS
"Fitness 0" 1.0 0 -5298144 true "" "plot count cells with [ fitness = 0 ]"
"Fitness 1" 1.0 0 -1184463 true "" "plot count cells with [ fitness = 1 ]"
"Fitness 2.5" 1.0 0 -13345367 true "" "plot count cells with [ fitness = 2.5 ]"
"Fitness 3" 1.0 0 -13840069 true "" "plot count cells with [ fitness = 3 ]"
"All cells" 1.0 0 -16777216 true "" "plot count cells"
